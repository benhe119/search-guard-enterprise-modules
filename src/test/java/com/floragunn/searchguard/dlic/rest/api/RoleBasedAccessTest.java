...
public class RoleBasedAccessTest extends AbstractRestApiUnitTest {
	...
	public void testActionGroupsApi() throws Exception {
		...
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		Assert.assertEquals(settings.getAsArray("sg_all_access.users")[0], "nagilum");
		Assert.assertEquals(settings.getAsArray("sg_role_starfleet_library.backendroles")[0], "starfleet*");
		Assert.assertEquals(settings.getAsArray("sg_zdummy_all.users")[0], "bug108");
		...
		Assert.assertEquals(settings.getAsArray("CLUSTER_MONITOR")[0], "cluster:monitor/*");
		// new format for action groups
		Assert.assertEquals(settings.getAsArray("CRUD.permissions")[0], "READ");
		...
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		Assert.assertEquals(settings.getAsArray("sg_role_starfleet_captains.cluster")[0], "cluster:monitor*");
		...
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		Assert.assertEquals(settings.getAsArray("sg_role_starfleet_captains.indices.hulla.dulla")[0], "blafasel");
