...
public class SgConfigApiTest extends AbstractRestApiUnitTest {
    ...
    public void testSgConfigApiWrite() throws Exception {
        ...
        response = rh.executePutRequest("/_searchguard/api/sgconfig/searchguard_xxx", FileHelper.loadFile("restapi/sgconfig.json"), new Header[0]);
        Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
        
        response = rh.executePutRequest("/_searchguard/api/sgconfig/searchguard", FileHelper.loadFile("restapi/sgconfig.json"), new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        ...
        response = rh.executePatchRequest("/_searchguard/api/sgconfig", "[{\"op\": \"replace\",\"path\": \"/searchguard/dynamic/hosts_resolver_mode\",\"value\": \"other\"}]", new Header[0]);
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
