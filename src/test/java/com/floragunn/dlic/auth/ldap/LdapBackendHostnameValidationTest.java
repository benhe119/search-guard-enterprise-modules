...
public class LdapBackendHostnameValidationTest {
    
    public static void main(String[] args) throws Throwable {
        try {
            System.out.println("LdapBackendHostnameValidationTest: "+Arrays.toString(args));
    ...
    public static void testHostnameVerification(boolean defineDisableEndpointIdentification, 
        ...
        final LdapUser user = (LdapUser) new LDAPAuthenticationBackend(settings, configPath).authenticate(new AuthCredentials("jacksonm", "secret"
                .getBytes(StandardCharsets.UTF_8)));

        new LDAPAuthorizationBackend(settings, configPath).fillRoles(user, null);
