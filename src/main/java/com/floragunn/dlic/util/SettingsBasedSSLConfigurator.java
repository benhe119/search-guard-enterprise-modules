    ...
    private void initFromKeyStore() throws SSLConfigException {
        ...
        try {
            ...
            throw new SSLConfigException("Error loading trust store from "
                    + settings.get(SSLConfigConstants.SEARCHGUARD_SSL_TRANSPORT_TRUSTSTORE_FILEPATH), e);
        }
        ...
        try {
            ...
            throw new SSLConfigException("Error loading key store from "
                    + settings.get(SSLConfigConstants.SEARCHGUARD_SSL_TRANSPORT_KEYSTORE_FILEPATH), e);
        }

        String keyStorePassword = settings.get(SSLConfigConstants.SEARCHGUARD_SSL_TRANSPORT_KEYSTORE_PASSWORD,
                SSLConfigConstants.DEFAULT_STORE_PASSWORD);
