...
public class ActionGroupsApiAction extends PatchableResourceApiAction {
    ...
    @Override
    protected void handleGet(final RestChannel channel, RestRequest request, Client client, Builder additionalSettings) throws IOException {
        ...
        final Settings configurationSettings = filterActionGroupsByApplication(loadAsSettings(getConfigName(), true).v2(), application, negate);
    ...
    private Settings filterActionGroupsByApplication(Settings settings, String application, boolean negate) {
        ...
        for (Map.Entry<String, Settings> entry : settings.getAsGroups(true).entrySet()) {
            List<String> matchingPermissions = getMatchingPermissions(settings, entry.getKey(), application, entry.getValue(), negate);
    ...
    private List<String> getMatchingPermissions(Settings settings, String key, String application, Settings subSettings, boolean negate) {
    ...
    private boolean containsActionByApplication(Settings settings, String key, String application, boolean negate, int maxRecursionDepth) {
    ...
    private boolean matchPermission(Settings settings, String key, String permission, String application, boolean negate, int maxRecursionDepth) {
        if (permission == null) {
            return false;
        }
        ...
        if (!key.equals(permission) && (settings.hasValue(permission) || settings.hasValue(permission + ".permissions"))) {
